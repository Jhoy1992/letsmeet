...
//     return done(null, { name: "Jhonatan", mail: "jhonatanascari@gmail.com" });
...
function setupLTI(ltiConfig) {
  ...
  ltiConfig.nonceStore = new imsLti.Stores.RedisStore(
    ltiConfig.consumerKey,
    redisClient
  );
  ltiConfig.passReqToCallback = true;

  const ltiStrategy = new LTIStrategy(ltiConfig, (req, lti, done) => {
...
async function setupAuth() {
  ...
  // app.post(
  //   "/auth/login",
  //   passport.authenticate("local", { failureRedirect: "/erro" }),
  //   function (req, res) {
  //     console.log("Merda de barrrrrrrrrrrrrrrrrrrrrrr");
