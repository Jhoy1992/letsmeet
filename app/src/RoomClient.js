...
let requestTimeout, transportOptions, lastN, mobileLastN;

if (process.env.NODE_ENV !== "test") {
  ({ requestTimeout, transportOptions, lastN, mobileLastN } = window.config);
}
...
export default class RoomClient {
  ...
  constructor({
    ...
    // Max spotlights
    if (device.platform === "desktop") this._maxSpotlights = lastN;
    else this._maxSpotlights = mobileLastN;
  ...
  async login(username, password, roomId = this._roomId) {
    try {
      ...
      this.receiveLoginData(response.data);
      return true;
    } catch (error) {
      logger.error('handleLogin() [error:"%o"]', error.message);
      ...
      store.dispatch(
        requestActions.notify({
          ...
          text: intl.formatMessage({
            id: "login.error",
            defaultMessage: "An error occurred on authenticating, try again",
          }),
        })
      );
  ...
  async logout(roomId = this._roomId) {
    try {
      const { data } = await api.get("/auth/logout");

      if (data.loggedOut) {
        this.receiveLogoutData();
      }
    } catch (error) {
      store.dispatch(
        requestActions.notify({
          ...
          text: intl.formatMessage({
            id: "logout.error",
            defaultMessage: "An error occurred on logout, try again",
          }),
        })
      );
    }
  ...
  receiveLoginData(data) {
    logger.debug('receiveLoginData() | [data:"%o"]', data);
  ...
  receiveLogoutData() {
    logger.debug("receiveLogoutData()");
  ...
  async sendChatMessage(chatMessage) {
    ...
    try {
      store.dispatch(chatActions.addUserMessage(chatMessage.text));
  ...
  async shareFiles(files) {
    ...
    createTorrent(files, (err, torrent) => {
      ...
      if (existingTorrent) {
        ...
        store.dispatch(
          fileActions.addFile(this._peerId, existingTorrent.magnetURI)
        );
      ...
      this._webTorrent.seed(
        ...
        (newTorrent) => {
          ...
          store.dispatch(
            fileActions.addFile(this._peerId, newTorrent.magnetURI)
          );
    ...
    try {
      for (const consumer of this._consumers.values()) {
        if (consumer.kind === "video") {
          ...
          else await this._pauseConsumer(consumer);
        }
      }
    } catch (error) {
  ...
  async updateWebcam({
    ...
    try {
      ...
      const { resolution, frameRate } = store.getState().settings;
  ...
  async _joinRoom({ joinVideo }) {
    ...
    try {
      ...
      if (this._produce) {
        ...
        if (joinVideo) this.updateWebcam({ start: true });
      }
