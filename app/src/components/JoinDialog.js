...
const DialogTitle = withStyles(styles)((props) => {
  ...
  const roomId = useParams().id.toLowerCase();
  ...
  const handleLogin = async () => {
    if (loggedIn) {
      await roomClient.logout();
      return;
    }

    history.push(`/login?roomId=${roomId}`);
  };
...
const JoinDialog = ({
  ...
  //verificar aqui se a sala existe e aproveitar pra pegar as informações da sessão
  ...
  const handleKeyDown = (event) => {
    ...
    switch (key) {
      case "Enter":
      case "Escape": {
        if (displayName === "") changeDisplayName("Guest");
      ...
      >
        ...
        <DialogContent>
          ...
          <DialogContentText gutterBottom>
            <FormattedMessage
              id="room.setYourName"
              defaultMessage={`Set your name for participation, 
