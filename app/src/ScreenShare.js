...
export default class ScreenShare {
  static create(device) {
    if (device.platform !== "desktop") return new DefaultScreenShare();
    else {
      switch (device.flag) {
        case "firefox": {
          if (device.version < 66.0) return new FirefoxScreenShare();
          else return new DisplayMediaScreenShare();
        }
        case "safari": {
          if (device.version >= 13.0) return new DisplayMediaScreenShare();
          else return new DefaultScreenShare();
        }
        case "chrome":
        case "chromium":
        case "opera":
