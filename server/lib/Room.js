...
class Room extends EventEmitter {
  ...
  static async create({ mediasoupWorkers, roomId }) {
    ...
    for (const worker of shuffledWorkers) {
      ...
      if (!firstRouter) firstRouter = router;
  ...
  async _handleSocketRequest(peer, request, cb) {
    ...
    switch (request.method) {
      ...
      case "join": {
        ...
        const { displayName, picture, rtpCapabilities } = request.data;
  ...
  async _createConsumer({ consumerPeer, producerPeer, producer }) {
    ...
    try {
      await this._request(consumerPeer.socket, "newConsumer", {
  ...
  _getPeersWithPermission(
    ...
    return Object.values(this._peers).filter(
      (peer) =>
        peer.joined === joined &&
        peer !== excludePeer &&
        peer.roles.some((role) => roomPermissions[permission].includes(role))
    );
  }
  ...
  async _pipeProducersToNewRouter() {
    const peersToPipe = Object.values(this._peers).filter(
      (peer) => peer.routerId !== this._currentRouter.id
    );
  ...
  async _getRouterId() {
    if (this._currentRouter) {
      const routerLoad = Object.values(this._peers).filter(
        (peer) => peer.routerId === this._currentRouter.id
      ).length;
